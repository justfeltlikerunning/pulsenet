...
- **Multi-agent pipelines** — `@Bayou @Wesley analyze this data` runs Bayou first, then Wesley with Bayou's response as context
...
|------|------------|---------|---------------|
| **T0** | Task only | "query Asset_sv WHERE isactive=1" | ~95% fewer tokens |
...
GET /api/messages/:conversationId?agent=bayou&last=5
...
```
@Bayou query active wells          → T0: task sent directly, no history
@Wesley what did bayou find         → T1: task + metadata, agent fetches context on demand
@Bayou @Wesley brainstorm together  → T2: full conversation history included
...
```
@Bayou @Wesley analyze treatment data
```

1. Bayou receives the message, processes it
2. Wesley receives the message + Bayou's response as additional context
