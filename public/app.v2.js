...
function connectSSE() {
  ...
  sse.addEventListener('pipeline_progress', e => {
    ...
    try {
      const data = JSON.parse(e.data);
      const { conversationId, step, total, current, completed } = data;
      ...
      const completedNames = completed.map(n => {
        const a = agentByName(n);
        return a ? a.name : n;
      }).join(' ✅ ');
      const currentName = agentByName(current)?.name || current;
      el.innerHTML = `<span class="pipeline-label">⚡ Pipeline step ${step}/${total} — waiting for ${escHtml(currentName)}${completedNames ? ' (done: ' + escHtml(completedNames) + ')' : ''}</span>`;
      scrollToBottom();
    } catch(_) {}
  });
