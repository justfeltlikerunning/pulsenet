...
// ─── Agent Registry ───────────────────────────────────────────────────────────
const AGENTS = [
  { id: 'agent-1',      name: 'Agent-1',      color: '#5b7fff', role: 'Coordinator' },
  { id: 'agent-2',      name: 'Agent-2',       color: '#4ade80', role: 'Data Agent' },
  { id: 'agent-5',     name: 'Agent-5',      color: '#f59e0b', role: 'Scorecard' },
  { id: 'agent-4',   name: 'Agent-4',    color: '#10b981', role: 'Fleet SRE' },
  { id: 'agent-3',   name: 'Agent-3',    color: '#ef4444', role: 'Win Monitor' },
  { id: 'agent-7',    name: 'Agent-7',     color: '#a78bfa', role: 'Bible Study' },
  { id: 'agent-6',      name: 'Agent-6',       color: '#ec4899', role: 'Memory/Brain' },
  { id: 'agent-8', name: 'Agent-8',  color: '#6366f1', role: 'Security' },
  { id: 'agent-9',      name: 'Agent-9',       color: '#14b8a6', role: 'Builder' },
  { id: 'mac-host', name: 'Mac-Host',  color: '#22c55e', role: 'macOS Builder' },
];

// Sidebar group presets
const FILTER_GROUPS = [
  { id: 'all',      label: 'All',      agents: [] },
  { id: 'work',     label: 'Work',     agents: ['agent-2','agent-5','agent-4','agent-3','agent-9'] },
  { id: 'personal', label: 'Personal', agents: ['agent-7'] },
  { id: 'security', label: 'Security', agents: ['agent-8'] },
  { id: 'brain',    label: 'Brain',    agents: ['agent-6','agent-1'] },
];
...
let currentComposeMode = localStorage.getItem('pulsenet_compose_mode') || 'RUN';

// ─── Multi-hub client failover ───────────────────────────────────────────────
const PRIMARY_ORIGIN = window.location.origin;
const HUB_ORIGINS = (() => {
  const configured = (localStorage.getItem('pulsenet_hubs') || '')
    .split(',').map(s => s.trim()).filter(Boolean);
  const defaults = [PRIMARY_ORIGIN, /* configured via FEDERATION_PEERS env */];
  // Optional additional hubs should be injected via localStorage/config, not hardcoded.
  for (const c of configured) if (!defaults.includes(c)) defaults.push(c);
  return defaults;
})();
...
async function apiFetch(input, init) {
  ...
  let lastErr = null;
  for (const base of HUB_ORIGINS) {
    const target = base.replace(/\/$/, '') + url;
    try {
      const resp = await fetch(target, init);
      if (resp.ok || resp.status < 500) {
        if (base !== PRIMARY_ORIGIN) {
          console.warn('[HubFailover] API served by backup hub:', base);
        }
        return resp;
      }
  ...
  throw lastErr || new Error('All hubs unavailable for ' + url);
}
...
function applyComposeModeUI() {
  const mode = (currentComposeMode || 'RUN').toUpperCase();
  const chips = [dom.modeRun, dom.modePlan, dom.modeStatus].filter(Boolean);
  for (const c of chips) c.classList.toggle('active', c.dataset.mode === mode);
  if (dom.modeIndicator) dom.modeIndicator.textContent = mode + ' mode';
  if (dom.messageInput) {
    const p = mode === 'RUN' ? 'Message... (@ to mention agents)'
      : mode === 'PLAN' ? 'Plan: ask for strategy/specs'
      : 'Status: ask for diagnostics/status';
    dom.messageInput.placeholder = p;
  }
}

function wireComposeMode() {
  const chips = [dom.modeRun, dom.modePlan, dom.modeStatus].filter(Boolean);
  chips.forEach(chip => chip.addEventListener('click', () => {
    currentComposeMode = chip.dataset.mode || 'RUN';
    localStorage.setItem('pulsenet_compose_mode', currentComposeMode);
    applyComposeModeUI();
  }));
  applyComposeModeUI();
}

function withComposeModePrefix(text) {
  const raw = String(text || '').trim();
  if (!raw) return raw;
  if (/^(RUN|EXECUTE|PLAN|STATUS|HEARTBEAT)\s*:/i.test(raw)) return raw;
  return `${currentComposeMode}: ${raw}`;
}
...
function showConvContextMenu(e, conv) {
  ...
  menu.addEventListener('click', async (ev) => {
    ...
    if (action === 'rename') {
      ...
      await apiFetch(`/api/conversations/${encodeURIComponent(conv.id)}`, {
        method: 'PATCH',
      ...
      await apiFetch(`/api/conversations/${encodeURIComponent(conv.id)}`, { method: 'DELETE' });
...
async function openConversation(convId) {
  ...
  try {
    const resp = await apiFetch(`/api/messages/${encodeURIComponent(convId)}`);
...
function renderChatHeader(conv) {
  ...
  dom.chatParticipants.querySelectorAll('.chip-remove').forEach(btn => {
    btn.addEventListener('click', async (e) => {
      ...
      try {
        await apiFetch(`/api/conversations/${encodeURIComponent(currentConvId)}/participants/${encodeURIComponent(agentId)}`, {
          method: 'DELETE',
        });
...
function appendMessage(msg, consecutive = false) {
  ...
  if (isSystem) {
    // Route hints — show briefly, auto-fade
    if (msg.type === 'route_hint') {
      wrapper.className = 'msg-wrapper system route-hint';
      wrapper.innerHTML = `<div class="route-hint">${escHtml(msg.body || '')}</div>`;
      dom.messages.appendChild(wrapper);
      // Auto-remove after 10 seconds
      setTimeout(() => { if (wrapper.parentNode) wrapper.remove(); }, 10000);
      return;
    }
...
function renderBurnMonitor(data) {
  ...
  const sigs = fleet.errorSignatures || {};

  // Color helpers
  const costCol = v => v > 50 ? '#eab308' : v > 20 ? '#f1f5f9' : '#22c55e';
  const errCol = v => v > 20 ? '#ef4444' : v > 5 ? '#eab308' : '#f1f5f9';

  // Summary cards
  let html = `<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:8px;margin-bottom:14px;">
    <div style="background:#141a2b;border:1px solid #2a3354;border-radius:10px;padding:10px;text-align:center;">
      <div style="font-size:11px;color:#9aa1af;text-transform:uppercase;letter-spacing:.05em;">Fleet Cost</div>
      <div style="font-size:20px;font-weight:700;color:${costCol(fleet.totalCost||0)}">$${(fleet.totalCost||0).toFixed(2)}</div>
    </div>
    <div style="background:#141a2b;border:1px solid #2a3354;border-radius:10px;padding:10px;text-align:center;">
      <div style="font-size:11px;color:#9aa1af;text-transform:uppercase;letter-spacing:.05em;">Errors</div>
      <div style="font-size:20px;font-weight:700;color:${errCol(fleet.totalErrors||0)}">${fleet.totalErrors||0}</div>
    </div>
    <div style="background:#141a2b;border:1px solid ${burns.length?'#ef4444':'#2a3354'};border-radius:10px;padding:10px;text-align:center;">
      <div style="font-size:11px;color:#9aa1af;text-transform:uppercase;letter-spacing:.05em;">Active Burns</div>
      <div style="font-size:20px;font-weight:700;color:${burns.length?'#ef4444':'#22c55e'}">${fleet.burnCount||0}</div>
    </div>
    <div style="background:#141a2b;border:1px solid #2a3354;border-radius:10px;padding:10px;text-align:center;">
      <div style="font-size:11px;color:#9aa1af;text-transform:uppercase;letter-spacing:.05em;">Wasted</div>
      <div style="font-size:20px;font-weight:700;color:${(fleet.burnCost||0)>0?'#ef4444':'#22c55e'}">$${(fleet.burnCost||0).toFixed(2)}</div>
    </div>
    <div style="background:#141a2b;border:1px solid #2a3354;border-radius:10px;padding:10px;text-align:center;">
      <div style="font-size:11px;color:#9aa1af;text-transform:uppercase;letter-spacing:.05em;">RPM</div>
      <div style="font-size:20px;font-weight:700;">${rate.rpm||0}<span style="font-size:12px;color:#9aa1af">/${rate.rpm_limit||0}</span></div>
    </div>
  </div>`;

  // Error signatures
  const sigKeys = Object.keys(sigs).sort((a,b) => sigs[b]-sigs[a]);
  if (sigKeys.length) {
    html += `<div style="margin-bottom:12px;display:flex;flex-wrap:wrap;gap:4px;">`;
    for (const s of sigKeys) {
      html += `<span style="display:inline-flex;align-items:center;gap:3px;background:#1a2332;border:1px solid #2a3354;border-radius:99px;padding:2px 8px;font-size:11px;font-family:monospace;color:#9aa1af;">
        <span style="background:rgba(255,255,255,.08);border-radius:99px;padding:0 5px;font-weight:600;color:#f1f5f9;font-size:10px;">${sigs[s]}</span>
        ${escHtml(s.replace(/_/g,' '))}
      </span>`;
    }
    html += `</div>`;
  }
  ...
  // Active burns alert
  if (burns.length) {
    html += `<div style="margin-bottom:12px;padding:10px;background:rgba(239,68,68,.1);border:1px solid rgba(239,68,68,.3);border-radius:10px;">
      <div style="font-weight:700;color:#ef4444;margin-bottom:6px;">🔥 ${burns.length} Active Burn${burns.length>1?'s':''}</div>`;
    for (const b of burns.slice(0,5)) {
      html += `<div style="font-size:12px;padding:2px 0;">
        <span style="color:#f97316;font-weight:600;">${escHtml(b.agent)}</span>/<span style="color:#9aa1af">${escHtml(b.displayName||'')}</span>
        — <span style="color:#ef4444">$${(b.cost||0).toFixed(2)}</span>, ${b.errorCount||0} errors
        <span style="color:#64748b;font-size:10px;margin-left:4px;">${escHtml((b.burnReason||'').replace(/_/g,' '))}</span>
      </div>`;
    }
    html += `</div>`;
  }
  ...
  // Agent table
  const agentArr = Object.entries(agents).sort((a,b) => (b[1].totalCost||0) - (a[1].totalCost||0));
  if (agentArr.length) {
    const maxCost = agentArr[0][1].totalCost || 1;
    html += `<div style="margin-bottom:12px;">
      <div style="font-size:11px;color:#9aa1af;text-transform:uppercase;letter-spacing:.06em;margin-bottom:6px;font-weight:600;">Cost by Agent</div>`;
    for (const [id, a] of agentArr) {
      const pct = maxCost > 0 ? (a.totalCost / maxCost * 100) : 0;
      const barCol = a.burnCount > 0 ? '#ef4444' : a.totalErrors > 5 ? '#eab308' : '#22c55e';
      html += `<div style="display:flex;align-items:center;gap:8px;padding:3px 0;font-size:12px;">
        <span style="min-width:80px;font-weight:600;">${escHtml(a.name)}</span>
        <div style="flex:1;height:5px;background:#0c1018;border-radius:3px;overflow:hidden;">
          <div style="height:100%;width:${pct}%;background:${barCol};border-radius:3px;transition:width .5s;"></div>
        </div>
        <span style="min-width:55px;text-align:right;font-family:monospace;font-size:11px;color:${(a.totalCost||0)>10?'#eab308':'#f1f5f9'}">$${(a.totalCost||0).toFixed(2)}</span>
        <span style="min-width:40px;text-align:right;font-family:monospace;font-size:11px;color:#9aa1af">${a.totalMessages||0}m</span>
        ${a.totalErrors > 0 ? `<span style="min-width:35px;text-align:right;font-family:monospace;font-size:10px;color:#ef4444">${a.totalErrors}e</span>` : '<span style="min-width:35px"></span>'}
      </div>`;
    }
    html += `</div>`;
  }
  ...
  // Top sessions table
  if (sessions.length) {
    html += `<div style="font-size:11px;color:#9aa1af;text-transform:uppercase;letter-spacing:.06em;margin-bottom:6px;font-weight:600;">Top Sessions (${sessions.length})</div>
    <div style="max-height:250px;overflow-y:auto;">
    <table style="width:100%;font-size:11px;border-collapse:collapse;">
      <thead><tr style="color:#64748b;border-bottom:1px solid #2a3354;">
        <th style="text-align:left;padding:3px 6px;">Agent</th>
        <th style="text-align:left;padding:3px 6px;">Session</th>
        <th style="text-align:right;padding:3px 6px;">Cost</th>
        <th style="text-align:right;padding:3px 6px;">Msgs</th>
        <th style="text-align:right;padding:3px 6px;">Errs</th>
      </tr></thead><tbody>`;
    for (const s of sessions.slice(0,30)) {
      const burnMark = s.isBurn ? ' 🔥' : '';
      const rowBg = s.isBurn ? 'background:rgba(239,68,68,.08);' : '';
      html += `<tr style="border-bottom:1px solid rgba(255,255,255,.03);${rowBg}">
        <td style="padding:3px 6px;color:#f97316;font-weight:600;">${escHtml(s.agent||'')}</td>
        <td style="padding:3px 6px;color:#9aa1af;max-width:150px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;" title="${escHtml(s.sessionKey||s.displayName||'')}">${escHtml(s.displayName||'')}${burnMark}</td>
        <td style="text-align:right;padding:3px 6px;font-family:monospace;color:${(s.cost||0)>5?'#eab308':'#f1f5f9'}">$${(s.cost||0).toFixed(2)}</td>
        <td style="text-align:right;padding:3px 6px;font-family:monospace;">${s.messages||0}</td>
        <td style="text-align:right;padding:3px 6px;font-family:monospace;color:${(s.errorCount||0)>0?'#ef4444':'#64748b'}">${s.errorCount||0}</td>
      </tr>`;
    }
    html += `</tbody></table></div>`;
  }

  html += `<div style="margin-top:10px;font-size:10px;color:#475569;text-align:right;">
    Scanned: ${escHtml(data.scannedAt ? new Date(data.scannedAt).toLocaleTimeString() : '?')} · Updated: ${new Date(data.updatedAt||Date.now()).toLocaleTimeString()}
  </div>`;

  dom.burnContent.innerHTML = html;
}
...
async function loadAgentHistory(agentId) {
  try {
    const resp = await apiFetch(`/api/agents/${agentId}/history`);
...
async function sendNewChat() {
  const bodyRaw = dom.newChatInput.value.trim();
  const body = withComposeModePrefix(bodyRaw);
  if (!bodyRaw) { dom.newChatInput.focus(); return; }
...
async function sendMessage() {
  ...
  const bodyRaw = dom.messageInput.value.trim();
  const body = withComposeModePrefix(bodyRaw);
  if (!bodyRaw && !pendingFiles.length) return;
  ...
  while ((match = mentionRegex.exec(bodyRaw)) !== null) {
    ...
    if (agent) {
      ...
      if (!currentParticipants.has(agent.id)) {
        try {
          await apiFetch(`/api/conversations/${encodeURIComponent(currentConvId)}/participants`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
  ...
  // Normalize "@ all" (with space) to "@all"
  const normalizedBodyRaw = bodyRaw.replace(/@\s+all\b/gi, '@all');
  const hasAll = /@\s*all\b/i.test(normalizedBodyRaw);
  let targets;
  if (hasAll) {
    ...
    if (namedAgents.length > 0) {
      // Coordinator pattern: @all @Agent-1 → supporters go first, Agent-1 summarizes last
      const rest = allParticipants.filter(p => !namedAgents.includes(p.toLowerCase()));
      targets = [...rest, ...namedAgents];
    } else {
  ...
  if (pendingFiles.length) {
    ...
    for (const file of pendingFiles) {
      ...
      isFirstFile = false;
      try {
        const uresp = await apiFetch('/api/upload', { method: 'POST', body: formData });
...
async function loadConversations() {
  try {
    ...
    conversations = await Promise.all(raw.map(async conv => {
      try {
        const pResp = await apiFetch(`/api/conversations/${encodeURIComponent(conv.id)}/participants`);
...
function connectSSE() {
  ...
  // Pipeline progress indicator (visual upgrade)
  let _pipelineAgents = []; // track full agent list for rendering
  sse.addEventListener('pipeline_progress', e => {
    markSseActivity();
    try {
      const data = JSON.parse(e.data);
      const { conversationId, step, total, current, completed, agents } = data;
      if (conversationId !== currentConvId) return;
      
      // Track agents list (first event may include full list)
      if (agents && agents.length) _pipelineAgents = agents;
      
      let el = document.getElementById('pipeline-progress');
      if (!el) {
      ...
      const currentName = agentByName(current)?.name || current;
      
      // Build agent pills
      let agentPills = '';
      const allAgents = _pipelineAgents.length ? _pipelineAgents : [...completed, current];
      for (const name of allAgents) {
        const displayName = agentByName(name)?.name || name;
        let state = 'waiting';
        let icon = '\u23f3';
        if (completed.includes(name)) { state = 'done'; icon = '\u2705'; }
        else if (name === current) { state = 'active'; icon = '\u26a1'; }
        agentPills += '<span class="pipeline-agent ' + state + '">' + icon + ' ' + escHtml(displayName) + '</span>';
      }
      
      el.innerHTML = 
        '<div class="pipeline-header">' +
          '<span class="pipeline-title">\u26a1 Fleet Pipeline</span>' +
          '<span class="pipeline-step-count">' + step + ' / ' + total + ' agents</span>' +
        '</div>' +
        '<div class="pipeline-bar"><div class="pipeline-bar-fill" style="width:' + pct + '%"></div></div>' +
        '<div class="pipeline-agents">' + agentPills + '</div>';
  ...
  sse.addEventListener('typing_stop', e => {
    try {
      ...
      // Remove pipeline progress if all agents done
      const el = document.getElementById('pipeline-progress');
      if (el && (!typingAgents[data.conversationId] || typingAgents[data.conversationId].size === 0)) {
        el.remove();
      }
    } catch(_) {}
  });
...
async function catchUpMessages() {
  ...
  try {
    ...
    const resp = await apiFetch('/api/messages/' + encodeURIComponent(currentConvId) + '?since=' + since);
...
async function deleteMessage(msgId) {
  ...
  try {
    const res = await apiFetch('/api/messages/' + encodeURIComponent(msgId), { method: 'DELETE' });
    if (res.ok) {
...
function showUndoToast(msgId, convId, windowMs) {
  ...
  toast.querySelector('#undo-send-btn').addEventListener('click', async () => {
    clearTimeout(timer);
    try {
      const resp = await apiFetch('/api/messages/' + encodeURIComponent(msgId), { method: 'DELETE' });
      const result = await resp.json();
      if (result.ok) {
        // Remove from UI
