...
try {
  ...
  webpush.setVapidDetails('mailto:admin@example.local', VAPID_PUBLIC_KEY, VAPID_PRIVATE_KEY);
...
// ─── Agent registry ───────────────────────────────────────────────────────────
const AGENTS = [
  { name: 'agent-1',      label: 'Agent-1',      color: '#5b7fff' },
  { name: 'agent-2',      label: 'Agent-2',       color: '#4ade80' },
  { name: 'agent-5',     label: 'Agent-5',      color: '#f59e0b' },
  { name: 'agent-4',   label: 'Agent-4',    color: '#10b981' },
  { name: 'agent-3',   label: 'Agent-3',    color: '#ef4444' },
  { name: 'agent-7',    label: 'Agent-7',     color: '#a78bfa' },
  { name: 'agent-6',      label: 'Agent-6',       color: '#ec4899' },
  { name: 'agent-8', label: 'Agent-8',  color: '#6366f1' },
  { name: 'agent-9',      label: 'Agent-9',       color: '#14b8a6' },
  { name: 'mac-host', label: 'Mac-Host', color: '#22c55e' },
];
...
const DOMAIN_ROUTES = {
  agent-2:      /\b(tanks?|wells?|assets?|quer(?:y|ies)|sql|database|db|schema|injection.?points?|lat.?long|active.?count|operators?|districts?|run.?tickets?|wrx|labs?|samples?|treatments?|chemicals?|vendor|yourcompany|fleet-system|telemetry|controller|sensor-vendor)\b/i,
  agent-5:     /\b(scorecards?|score.?cards?|reports?|scheduled|cron.?jobs?|missing.?coords?|lat.?long.?score|weekly|daily.?report)\b/i,
  agent-3:   /\b(windows|task.?scheduler|vm.?tasks?|sftp|azure.?blob|scheduled.?tasks?|automation|agent-3)\b/i,
  agent-4:   /\b(fleet|gateways?|dashboard|prometheus|alerts?|infra|cpu|memory|disks?|uptime|health|sre|monitoring)\b/i,
  agent-8: /\b(security|patch(?:es|ing)?|vulnerabilit(?:y|ies)|audits?|ssh|firewalls?|ufw|fail2ban|cve|hardening)\b/i,
  agent-6:      /\b(brain|search|memory|knowledge|cross.?agent|correlation|semantic|vector-db|embeddings?)\b/i,
  agent-7:    /\b(church|bible|sermons?|scripture|study|curriculum|faith|prayer)\b/i,
  agent-9:      /\b(build|cod(?:e|ing)|projects?|git|repos?|deploy|dashboard.?ui|frontend|backend)\b/i,
};
...
function buildT1Metadata(conversationId, history) {
  ...
  const publicUrl = config.publicUrl || process.env.PUBLIC_URL || 'https://localhost';
  return {
...
async function checkHttpFallbackAlerts() {
  for (const [agent, stats] of Object.entries(dispatchStats)) {
    if (stats.consecutiveHttp >= CONSECUTIVE_HTTP_THRESHOLD) {
      ...
      // Alert Agent-4 via its agent-pulse WS connection
      const agent-4State = agentWsState['agent-4'];
      if (agent-4State && agent-4State.connected && agent-4State.ws) {
        const alertEnvelope = {
          protocol: 'pulse/1.0',
          id: uuidv4(),
          type: 'message',
        ...
        try {
          agent-4State.ws.send(JSON.stringify(alertEnvelope));
          console.log(`[DISPATCH:ALERT] Sent HTTP fallback alert for ${agent} to Agent-4`);
        } catch(e) {
          console.warn(`[DISPATCH:ALERT] Failed to alert Agent-4: ${e.message}`);
        }
      }
    }
  }
...
// ─── Agent-6 Brain Search ──────────────────────────────────────────────────────
async function searchAgent-6Brain(query, agentName, topK = 3) {
  try {
    const res = await fetch((config.agent-6BrainUrl || 'http://localhost:8900') + '/search', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    return `\n\n[Agent-6\'s Brain - relevant memories for ${agentName}:]\n${summaries}`;
  } catch(e) {
    return '';
  }
}
...
async function postToAgent(agentName, conversationId, messageText, history, previousContext, media, targetHint) {
  ...
  if (tier === 'T0') {
    ...
    const brainContext = await searchAgent-6Brain(userText, agentName);
...
async function handleRequest(req, res) {
  ...
  if (pathname.startsWith('/api/')) {
    ...
    // GET /api/burn-monitor — unified burn view (dashboard burns + ratelimit proxy)
    if (pathname === '/api/burn-monitor' && method === 'GET') {
      const DASH = process.env.DASHBOARD_URL || 'http://localhost:8880';
      try {
        const [burnRes, rateRes] = await Promise.allSettled([
          fetch(DASH + '/api/burns', { signal: AbortSignal.timeout(5000) }).then(r => r.json()),
          fetch(DASH + '/api/ratelimit', { signal: AbortSignal.timeout(3000) }).then(r => r.json()),
        ]);
        const burns = burnRes.status === 'fulfilled' ? burnRes.value : {};
        const rates = rateRes.status === 'fulfilled' ? rateRes.value : {};
        const u = (rates.unified || {}).sources || {};
        json(res, {
          ok: true,
          // Burns data
          fleet: burns.fleet || {},
          agents: burns.agents || {},
          sessions: (burns.sessions || []).slice(0, 30),
          burns: burns.burns || [],
          date: burns.date || null,
          scannedAt: burns.scannedAt || null,
          // Rate limiter data
          sources: u,
          rate: rates.rates || {},
...
// ─── Delayed dispatch with undo window ──────────────────────────────
      ...
      if (uniqueAgents.length <= 1) {
        ...
        // @Agent-2 @Agent-5 = Agent-2 runs first, Agent-5 gets Agent-2's response as context
