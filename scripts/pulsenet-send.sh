...
#
# Environment:
#   AGENT_NAME / MY_AGENT   — sender identity (default: ltdan)
#   PULSENET_URL            — server URL (default: http://192.168.1.9:3000)
...
PULSENET_URL="${PULSENET_URL:-${SHRIMPNET_URL:-http://192.168.1.9:3000}}"
PULSENET_TOKEN="${PULSENET_TOKEN:-${SHRIMPNET_TOKEN:-changeme}}"
...
usage() {
    cat >&2 << 'EOF'
Usage: shrimpnet-send.sh [options] <target> <message>
       shrimpnet-send.sh [options] --targets "a,b,c" <message>
       shrimpnet-send.sh [options] --broadcast <message>
...
Examples:
  shrimpnet-send.sh bayou "Query active tank count"
  shrimpnet-send.sh --targets "bayou,wesley" "Cross-check query" --type request
  shrimpnet-send.sh --broadcast "Schema v3 deployed" --conv alerts
EOF
    exit 1
}
...
if [[ "$BROADCAST" == "true" ]]; then
    AGENT_LIST=(ltdan greenbow bayou danwatch wesley feather jenny bubbawatch sully)
    # Remove self
...
if [[ -n "$CONV_ID_OVERRIDE" ]]; then
    ...
    # Map short names to channel IDs (same as shrimpnet-report.sh)
    declare -A CONV_MAP=(
...
for agent in "${AGENT_LIST[@]}"; do
    ...
    if [[ -n "$RESPONSE" ]] && (echo "$RESPONSE" | jq -e '.ok // .id' >/dev/null 2>&1); then
        echo -e "${GREEN}✓ Sent to ${agent} via ShrimpNet${NC}" >&2
        SENT=$((SENT + 1))
    else
...
TOTAL=$((SENT + FAILED))
if [[ $FAILED -eq 0 ]]; then
    echo -e "${GREEN}✅ All ${SENT} message(s) delivered to ShrimpNet${NC}" >&2
else
